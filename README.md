...
=3\frac{L_{m} }{L_{r}}pIm\left \{\Psi _{r}^*I_{s}\right \}=3\frac{L_{m} }{\sigma L_{r}L_{s}}pIm\left \{\Psi _{r}^*\Psi _{s}\right \}
```
其中 $`\sigma = 1 - \frac{L_{m}^2}{L_{s}L_{r}}`$ 。此外，通过在有功功率方程（1.16）中忽略铜损，可以得出一些近似的功率表达式，这些表达式能够简单地关联机器的功率、转矩和速度。这些简单的表达式总结在下表中，它们在进行快速计算时非常有用。需要强调的是，定子电压和转子电压之间的关系也在表中展示，这个关系是通过忽略定子和转子电阻以及漏感电压降得出的（见方程1.8和1.9）。需要注意的是，由于定子直接连接电网，因此可以假设定子电压$`V_{s}`$是恒定的的情况下，所需的转子电压幅度主要取决于转差率或速度。

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;因此，可以区分出四种可能的转矩（正或负）和速度（亚同步和超同步）的组合，导致双馈感应电机（DFIM）的四象限运行模式，如图1.6所示。请注意，只有在超同步模式下，当转差率为负时，定子和转子的有功功率才会表现出相同的符号。

||||
|---|---|---|
|$`P_{s}+P_{r}\cong P_{m}`$ |$`P_{r}\cong -sP_{s}`$|$`P_{m}\cong (1-s)P_{s}`$|
| $`\lvert \underline{V}_{r}\rvert \cong \lvert s\underline{V}_{s} \rvert`$ | | |
|$`P_{m} = T_{em}\frac{\omega_{m}}{p}`$ |$`P_{s}\cong T_{em}\frac{\omega_{s}}{p}`$|$`P_{r}\cong T_{em} \frac{\omega_{r}}{p} `$|
