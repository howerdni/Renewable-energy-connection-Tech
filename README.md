...
```
其中 $`\sigma = 1 - \frac{L_{m}^2}{L_{s}L_{r}}`$ 。此外，通过在有功功率方程（1.16）中忽略铜损，可以得出一些近似的功率表达式，这些表达式能够简单地关联机器的功率、转矩和速度。这些简单的表达式总结在表1.1中，它们在进行快速计算时非常有用。需要强调的是，定子电压和转子电压之间的关系也在表中展示，这个关系是通过忽略定子和转子电阻以及漏感电压降得出的（见方程1.8和1.9）。需要注意的是，由于定子直接连接电网，因此可以假设定子电压$`V_{s}`$是恒定的的情况下，所需的转子电压幅度主要取决于转差率或速度。
